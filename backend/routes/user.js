...
router.put('/update-profile', protect, async (req, res) => {
  ...
  const profileFields = { name, bio, location, skills, interests, socialLinks, profilePicture };
  ...
  try {
    ...
    user = await User.findByIdAndUpdate(req.user.id, { $set: profileFields }, { new: true, runValidators: true });
...
router.get('/:id/sessions', protect, async (req, res) => {
  try {
    const sessions = await Session.find({ $or: [{ mentor: req.params.id }, { mentee: req.params.id }] })
